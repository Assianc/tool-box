...
class WeChat:
    ...
    def GetSessionAmont(self, SessionItem):
        ...
        if matchobj:
            try:
                amount = int(
                    [i for i in SessionItem.GetChildren()[0].GetChildren() if type(i) == uia.uiautomation.TextControl][
                        0].Name)
            except:
                pass
        if amount:
    ...
    def GetLatestMessage(self, savepic=False):
        '''获取当前窗口中最新的一条聊天记录
        ...
        if MsgItems:
            # 获取最后一条聊天记录项
            latest_msg_item = MsgItems[-1]

            # 解析最新的一条聊天记录
            latest_msg = self._split(latest_msg_item)

            # 如果消息中包含图片，并且需要保存图片
            if latest_msg[1] == f"[{self._lang('图片')}]":
                if savepic:
                    # 保存图片逻辑
                    pass
...
class WeChatImage:
    ...
    def Save(self, savepath='', timeout=10):
        ...
        """
        if not savepath:
            savepath = os.path.join(os.getcwd(), '微信图片',
                                    f"微信图片_{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}.jpg")
